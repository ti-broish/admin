...
export default (props) => {
  ...
  const initResults = () => {
    ...
    for (const result of props.protocol.results.results) {
      ...
      resultsObj[`${result.party.id}m`] = emptyStrIfNull(
        result.machineVotesCount
      );
      resultsObj[`${result.party.id}nm`] = emptyStrIfNull(
    ...
    if ((allParties ? true : party.isFeatured) || party.id.toString() === '0') {
      const updateFieldStatus = (apiKey, resultSuffix) => {
        ...
        if (resultsData[`${party.id}${resultSuffix}`] === '')
          ...
          resultsData[`${party.id}${resultSuffix}`].toString()
        )
  ...
  const addStatusForResultField = (fieldName) => {
    if (formData[fieldName] === '') fieldStatus[fieldName] = { invalid: true };
    else if (
      formData[fieldName] !== zeroIfEmpty(props.protocol.results[fieldName])
    )
      ...
      <tr>
        ...
        <td>
          <input
            type="text"
            className={
              status.invalid ? 'invalid' : status.changed ? 'changed' : ''
            }
        ...
        <tr>
          ...
          <td>
            <input
              type="text"
              className={
                statusNM.invalid ? 'invalid' : statusNM.changed ? 'changed' : ''
              }
        ...
        <tr>
          ...
          <td>
            <input
              type="text"
              className={
                statusM.invalid ? 'invalid' : statusM.changed ? 'changed' : ''
              }
  ...
  const handleNumberChange = (e) => {
    const newValue = filterNumberFieldInput(
      e.target.value,
  ...
  const replaceProtocol = async () => {
    ...
    Object.keys(resultsData).forEach((key) => {
      if (key[key.length - 2] === 'n') {
        ...
        results[newKey].nonMachineVotesCount = !sectionData.isMachine
          ? null
          : parseInt(resultsData[key], 10);
      } else if (key[key.length - 1] === 'm') {
        let newKey = key.slice(0, key.length - 1);
        if (!results[newKey]) results[newKey] = {};
        results[newKey].machineVotesCount = !sectionData.isMachine
          ? null
          : parseInt(resultsData[key], 10);
      } else {
      ...
      <ProtocolInfoSection ref={ref}>
        ...
        <ProtocolDetails>
          ...
          <p style={{ fontSize: '20px' }}>
            {!sectionData.country
              ? null
              : ['Държава: ', <b>{sectionData.country}</b>, ', ']}
            {!sectionData.electionRegion
              ? null
              : ['Изборен район: ', <b>{sectionData.electionRegion}</b>, ', ']}
            <br />
            {!sectionData.municipality
              ? null
              : ['Община: ', <b>{sectionData.municipality}</b>, ', ']}
            {!sectionData.town
              ? null
              : ['Населено място: ', <b>{sectionData.town}</b>, ', ']}
            <br />
            {!sectionData.cityRegion
              ? null
              : ['Район: ', <b>{sectionData.cityRegion}</b>, ', ']}
            {!sectionData.address
              ? null
              : ['Локация: ', <b>{sectionData.address}</b>]}
          </p>
          <table>
            <tbody>
              <tr>
          ...
          <ProtocolDetailsTable>
            <tbody>
              <tr>
                <td>
                  2. Брой на гласувалите избиратели според положените подписи в
                ...
                <td>
                  <input
                    ...
                    name="votersCount"
                    className={
                      fieldStatus['votersCount'].invalid
                        ? 'invalid'
                        : fieldStatus['votersCount'].changed
                        ? 'changed'
                        : ''
                    }
          ...
          <ProtocolDetailsTable>
            <tbody>
              <tr>
                <td>
                  5. Брой намерени в избирателната кутия недействителни гласове
                  (бюлетини)
                </td>
                <td>
                  <input
                    ...
                    className={
                      fieldStatus['invalidVotesCount'].invalid
                        ? 'invalid'
                        : fieldStatus['invalidVotesCount'].changed
                        ? 'changed'
                        : ''
                    }
                    onChange={handleNumberChange}
              ...
              <tr>
                <td>
                  6.1. Брой на действителните гласове, подадени за кандидатските
                  листи на партии, коалиции и ИК
                </td>
                <td>
                  <input
                    type="text"
                    ...
                    className={
                      fieldStatus['validVotesCount'].invalid
                        ? 'invalid'
                        : fieldStatus['validVotesCount'].changed
                        ? 'changed'
                        : ''
                    }
                    onChange={handleNumberChange}
              ...
              {!sectionData.isMachine ? null : (
                <>
                  <tr>
                    <td>
                      6.2а. Брой на намерените в избирателна кутия дейстивтелни
                      гласове "Не подрекпям никого"
                    </td>
                    <td>
                      <input
                        type="text"
                        className={
                          fieldStatus[`party0nm`].invalid
                            ? 'invalid'
                            : fieldStatus[`party0nm`].changed
                            ? 'changed'
                            : ''
                        }
                        data-party-id={'0nm'}
                  ...
                  <tr>
                    <td>
                      6.2б. Брой гласували за "Не подкрепям никого" от машинното
                      гласуване
                    </td>
                    <td>
                      <input
                        type="text"
                        className={
                          fieldStatus[`party0m`].invalid
                            ? 'invalid'
                            : fieldStatus[`party0m`].changed
                            ? 'changed'
                            : ''
                        }
                        data-party-id={'0m'}
              ...
              <tr>
                <td>
                  6.2. Брой на действителните гласове "Не подкрепям никого"
                </td>
                <td>
                  <input
                    type="text"
                    className={
                      fieldStatus[`party0`].invalid
                        ? 'invalid'
                        : fieldStatus[`party0`].changed
                        ? 'changed'
                        : ''
                    }
                    data-party-id={'0'}
          ...
          <PartyResultsTable isMachine={sectionData.isMachine}>
            <tbody>
              {parties.map((party) =>
                !(
                  (allParties ? true : party.isFeatured) &&
                  party.id.toString() !== '0'
                )
                  ? null
                  : partyRow(party)
