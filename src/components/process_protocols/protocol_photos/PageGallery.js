...
export default (props) => {
  ...
  const movePageForwardPossible =
    selectedPhoto < props.protocol.pictures.length - 1
  ...
  const rotate = () => {
    ...
    if (pictures[selectedPhoto].rotation >= 360)
      pictures[selectedPhoto].rotation -= 360
    props.reorderPictures([...pictures])
  }
      ...
      {!selectedPhoto && selectedPhoto !== 0 ? null : (
        <>
          <SelectedPageControls>
            ...
            <SelectedPageButton
              disabled={!movePageBackPossible}
              onClick={movePageBack}
            >
              <FontAwesomeIcon icon={faStepBackward} />
            </SelectedPageButton>
            <SelectedPageButton
              disabled={!movePageForwardPossible}
